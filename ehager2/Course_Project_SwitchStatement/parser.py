...
grammar = """
    ...
    switch_statement = "switch" "(" simple_expression ")" "{" case_statement  "}"
    break_statement = "break";
    case_statement = "case" simple_expression ":" statement_list break_statement
    default_statement = "default" simple_expression ":" statement_list break_statement
...
def test_parse_statement():
    """
    statement = block_statement | if_statement | while_statement | function_statement | return_statement | print_statement | assignment_statement | switch_statement ;
    """
    ...
    #Switch statement
    assert(
       parse_statement(tokenize("switch(1){case 1: print(1); break}"))[0]
       == parse_switch_statement(tokenize("switch (1) {case 1: print(1); break}"))[0]
    )
...
def parse_case_statement(tokens):
    ...
    tokens = tokens[1:]
    pprint("Token now after the slicing of case")
    pprint(tokens[0]["tag"])

    #Now can grab the case condition
    caseCondition, tokens = parse_simple_expression(tokens)

    #Consume the :
    assert (tokens[0]["tag"] == ":")
    tokens = tokens[1:]

    pprint("AFTER CONSUMING THE :")
    pprint(tokens)

    #Now parse the block statement
    statementList, tokens = parse_statement_list(tokens)

    pprint("Case Condition: ")
    pprint(caseCondition)

    pprint("Statement List")
    pprint(statementList)

    pprint("Rest of the tokens")
    pprint(tokens)
...
def parse_switch_statement(tokens):
    ...
    #Need to check for the switch
    assert (tokens[0]["tag"] == "switch")
    
    #pprint(tokens)

    #Move the tokens forward
    tokens = tokens[1:]

    #pprint("TOKENS AFTER SLICING")
    #pprint(tokens)

    #Now check for the open parenthesis
    if (tokens[0]["tag"] != "("):
        raise Exception(f"Expected '(': {tokens[0]}")

    #Now need to parse the simple_expression with in the parenthesis as the argument
    #to the statement
    argument, tokens = parse_simple_expression(tokens[1:]) 

    #pprint(argument)

    #Now parsed print out the tokens
    pprint("TOKENS AFTER PARING THE SIMPLE EXPRESSION")
    pprint(tokens)
    ...
    #Check for the {
    if (tokens[1]["tag"] != "{"):
        raise Exception(f"Expected end brace: {tokens[1]}")

    #Now parse for each case statement embeded in the switch
    ast, case = parse_case_statement(tokens[2:])
    node = None
    tokens = None
...
def test_parse_switch_statement() :
    ...
    ast, _ = parse_switch_statement(tokenize("switch(1) {case 1: print(1); break}"))
    exit(0)
